...
# --- RAILROAD.synonyms_for (the module-level wrapper functions that used to
# front these -- equipment_terms_prompt, settings_prompt, wheel_arrangements_
# prompt, synonyms_for, valid_equipment_types, frequency_terms -- had zero
# production callers in either this repo or needlestack, only these tests;
# removed, tests now call the real Domain API directly) ---

def test_synonyms_for_known_term():
    syn = RAILROAD.synonyms_for("caboose")
...
def test_synonyms_for_via_synonym_input():
    ...
    syn = RAILROAD.synonyms_for("reefer")
    assert "refrigerator car" in syn
    assert "reefer" not in syn


def test_synonyms_for_is_case_insensitive():
    assert RAILROAD.synonyms_for("CABOOSE") == RAILROAD.synonyms_for("caboose")


def test_synonyms_for_unknown_term_empty():
    assert RAILROAD.synonyms_for("automobile") == []
...
def test_synonyms_for_empty_string():
    """Pins current behavior for an ambiguous input: no canonical type or
    synonym is ever the empty string, so this is a safe [] -- but it was
    previously untested, not deliberately pinned."""
    assert RAILROAD.synonyms_for("") == []
...
def test_frequency_terms_derive_from_equipment():
    ...
    assert set(RAILROAD.frequency_terms()) == RAILROAD.valid_subject_types


def test_valid_types_are_canonical_keys():
    assert RAILROAD.valid_subject_types == set(taxonomy.EQUIPMENT.keys())


def test_prompt_helpers_nonempty():
    assert "caboose" in RAILROAD.subject_types_prompt()
    assert RAILROAD.settings_prompt()
